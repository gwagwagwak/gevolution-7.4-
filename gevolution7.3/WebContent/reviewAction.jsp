...
<jsp:setProperty name="review" property="nickname"/>	<!--property의 id는 login.jsp에서 input의 name에서 받아온 id의 값을 member객체의  id안에 저장하게 된다. (이름이 같으니깐) -->
<jsp:setProperty name="review" property="r_content"/>
<jsp:setProperty name="review" property="game_name"/>
	...
	<%
		...
		//로그인을 한 상태
			...
			//제목을 입력한 경우
				...
				//DAO bdao = new BoardDAO();
				//int result = bdao.writeBoard(board.getTitle(), board.getNickname(), board.getContent(), board.getB_type());
					
				
				ReviewDAO rdao = new ReviewDAO();
				int result = rdao.writeReview(review.getNickname(), review.getR_content(), review.getGame_name(), review.getRating());
					
				if(result == -1){ 	//db오류 게시판 글쓰기 실패
					script.println("<script>");
					script.println("alert('리뷰작성에 실패했습니다.')");
					script.println("history.back()");
					script.println("</script>");
				}
				else  { //정상 글쓰기 성공
					...
					script.println("<script>");
					script.println("location.href= 'reviewListAction.do'");	//회원가입 성공시 login페이지로 이동함
					script.println("</script>");
				}
